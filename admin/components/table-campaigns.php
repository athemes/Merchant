...
foreach ( $options as $module_id => $option ) {
	...
	// Campaign key
	$campaign_key = '';
	if ( isset( $option['offers'] ) ) {
		$campaign_key = 'offers';
	} elseif ( isset( $option['rules'] ) ) {
		$campaign_key = 'rules';
	}

	// Campaigns array
	$campaigns = $option[ $campaign_key ] ?? array();
	if ( empty( $campaigns ) ) {
		continue;
	}
...
<div class="merchant__campaigns-table-nav">
    <div class="alignleft bulk-action">
        <select class="bulk-action-selector">
            ...
            <option value="enable"><?php echo esc_html__( 'Enable', 'merchant' ); ?></option>
            <option value="disable"><?php echo esc_html__( 'Disable', 'merchant' ); ?></option>
        </select>
...
<table class="wp-list-table widefat table-view-list merchant__campaigns-table js-campaigns-table">
    <thead>
        <tr>
            ...
            <th>
                <a href="#" class="merchant-sort" data-sort="campaign-name">
                    <?php echo esc_html__( 'Campaign Name', 'merchant' ); ?>
                    <?php echo wp_kses_post( $sorting_indicator_html ); ?>
                </a>
            </th>
            <th>
                <a href="#" class="merchant-sort" data-sort="module-name">
            ...
            <th>
                <a href="#" class="merchant-sort" data-sort="impressions">
                    <?php echo esc_html__( 'Impressions', 'merchant' ); ?>
                    <?php echo wp_kses_post( $sorting_indicator_html ); ?>
                </a>
            </th>
            <th>
                <a href="#" class="merchant-sort" data-sort="clicks">
                    <?php echo esc_html__( 'Clicks', 'merchant' ); ?>
                    <?php echo wp_kses_post( $sorting_indicator_html ); ?>
                </a>
            </th>
            <th>
                <a href="#" class="merchant-sort" data-sort="revenue">
                    <?php echo esc_html__( 'Revenue', 'merchant' ); ?>
                    <?php echo wp_kses_post( $sorting_indicator_html ); ?>
                </a>
            </th>
            <th>
                <a href="#" class="merchant-sort" data-sort="ctr">
                    <?php echo esc_html__( 'CTR', 'merchant' ); ?>
                    <?php echo wp_kses_post( $sorting_indicator_html ); ?>
                </a>
            </th>
            <th>
                <a href="#" class="merchant-sort" data-sort="orders">
    ...
    <tbody>
        ...
        foreach ( $campaigns_data as $module_index => $module ) :
            ...
            foreach ( $module['campaigns'] as $campaign_index => $campaign ) :
                $ctr_color = $campaign['ctr'] < 0 ? 'red' : 'green'; // Todo
                $ctr_icon  = $campaign['ctr'] < 0 ? '' : '＋';
                ?>
                <tr
                    data-module-id="<?php echo esc_attr( $module_id ); ?>"
                    data-campaign-key="<?php echo esc_attr( $campaign['key'] ?? '' ); ?>"
                    data-campaign-id="<?php echo esc_attr( $campaign_index ); ?>"
                    ...
                    <td class="merchant__impressions"><?php echo esc_html( number_format( $campaign['impression'] ) ); ?></td>
                    <td class="merchant__clicks"><?php echo esc_html( number_format( $campaign['clicks'] ) ); ?></td>
                    <td class="merchant__revenue"><?php echo wp_kses_post( wc_price( $campaign['revenue'] ) ); ?></td>
                    <td class="ç" style="color: <?php echo esc_attr( $ctr_color ); ?>;">
                        <?php echo esc_html( $ctr_icon ) . esc_html( $campaign['ctr'] . '%' ); ?>
                    </td>
                    <td class="merchant__orders"><?php echo esc_html( number_format( $campaign['orders'] ) ); ?></td>
