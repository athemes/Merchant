<?php $modules = Merchant_Admin_Modules::get_modules(); ?>
			...
			<div class="merchant-modules-list">
				...
				<?php foreach ( $modules as $data ) : ?>
					...
					<?php if ( ! empty( $data['modules'] ) ) : ?>
						...
						<?php foreach ( $data['modules'] as $module_id => $module ) : ?>

							<?php if ( ! Merchant_Modules::is_module_active( $module_id ) ) { continue; } ?>

							<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'merchant', 'module' => $module_id ), 'admin.php' ) ); ?>" class="merchant-modules-list-item">

								<div class="merchant-modules-list-item-icon">
									<?php if ( ! empty( $module['icon'] ) ) : ?>
										<?php echo wp_kses( $module['icon'], merchant_get_svg_args() ); ?>
									<?php else: ?>
