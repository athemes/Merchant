...
$product  = isset( $args['product'] ) ? wc_get_product( $args['product'] ) : wc_get_product();
?>
<div class="merchant-bogo">
	...
	echo esc_url( wc_get_cart_url() ); ?>">
		...
		foreach ( $args['offers'] as $key => $offer ): ?>
			<?php
			if ( isset( $offer['product']['is_purchasable'] ) && ! $offer['product']['is_purchasable'] ) {
				continue;
			} ?>
            <div class="merchant-bogo-offer" data-product="<?php
			echo esc_attr( get_the_ID() ) ?>" data-offer="<?php
			echo esc_attr( $key ); ?>">
                <div class="merchant-bogo-product-x">
                    ...
                    <div class="merchant-bogo-product">
						<?php
						echo wp_kses_post( $product->get_image( array( 90, 90 ) ) ); ?>
                        <div class="merchant-bogo-product-contents">
                            <p class="woocommerce-loop-product__title">
                                <a href="<?php
								echo esc_url( $product->get_permalink() ); ?>" target="_blank">
									<?php
									echo esc_html( $product->get_name() ); ?>
                                </a>
                            </p>
							<?php
							echo wp_kses( $product->get_price_html(), merchant_kses_allowed_tags( array( 'bdi' ) ) ); ?>
                        </div>
                    </div>
                ...
                <div class="merchant-bogo-product-y">
					...
					echo esc_attr( $product->get_id() ); ?>">
						...
						<?php
						$buy_product = $offer['customer_get_product_ids'] ? wc_get_product( $offer['customer_get_product_ids'] ) : null;
						if ( $buy_product ) {
							?>
                            <div class="merchant-bogo-product">
								<?php
								echo wp_kses_post( $buy_product->get_image( array( 90, 90 ) ) ); ?>
                                <div class="merchant-bogo-product-contents">
                                    <p class="woocommerce-loop-product__title">
                                        <a href="<?php
